...
import jatools.dataset.CrossField;
...
public class CrossIndexNode extends DatasetBasedNode {
    ...
    public Object getProperty(String prop, CallStack callstack, Interpreter interpreter) {
        ...
        if (col > -1) {
            Key key = this.getRoot().getScript().getKeyStack(0).getKey();
            Key key2 = this.getRoot().getScript().getKeyStack(1).getKey();

            return new CrossField(this.indexView, col, key, key2);
        } else {
