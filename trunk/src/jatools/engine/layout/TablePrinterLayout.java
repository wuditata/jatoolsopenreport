...
public class TablePrinterLayout extends AbstractPrinterLayout {
    ...
    public void doLayout() {
        ...
        int dy = rows.getPosition(grid.getRows().length()) - this.imageable.height;
        //     if (dy>0) {

        //    	rows.setSize(rows.length() -1,rows.getSize( rows.length()-1 )-dy );

        //            GridView remainings = grid;
        //
        //            int i = rows.length() - 1;
        //
        //            // 找出最后一个合法的行
        ...
        //                ;
        //
        //            GridViewSplitter splitter = new GridViewSplitter(grid);
        //            this.rootView = splitter.selectRowsBefore(i);
        //
        //            ((TablePrinter) getPrinter()).setRemainingView(remainings);
        //   }
        rootView.doLayout();
    }
