...
public class RowPanelPrinter extends AbstractPrinter {
    ...
    private boolean isTopMost() {
        if (layer == UNKNOWN) {
            ...
            for (int i = 0; i < c; i++) {
                if (getComponent().getChild(i) instanceof RowPanel) {
                    layer = OVERRIDE;
        ...
        for (int row = cell.row; row < (cell.row + cell.rowSpan); row++) {
            ...
            for (int col = cell.column; col < (cell.column + cell.colSpan); col++) {
                ...
                if (v != null) {
                    ...
                    if (tableLayout.getViewRowHeight(lastRow) < acell.rowSpan) {
                        tableLayout.setViewRowHeight(lastRow, acell.rowSpan);
                    }
                    ...
                    v = (AbstractView) printer.unitView(context.getScript(), v, tableLayout, true);
